...
export default function useDependencies(fieldName: string) {
  ...
  if (!form)
    throw new Error(
      'useDependencies should be used within <AutoForm>'
    )
  ...
  function getSourceValue(dep: Dependency<any>) {
    ...
    const [sourceLast, ...sourceInitial] = source
      .split('.')
      .toReversed()
    const [_targetLast, ...targetInitial] = (
      dep.targetField as string
    )
      .split('.')
      .toReversed()
    ...
    if (
      index >= 0 &&
      sourceInitial.join(',') === targetInitial.join(',')
    ) {
